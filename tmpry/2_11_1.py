...
class CVEDataProcessor:
    ...
    def extract_severity_info(self, cve_item):
        ...
        #severity = cve_item.get("impact", {}).get("baseMetricV3", {}).get("baseSeverity", "Unknown")
        #impact = cve_item.get("impact", {}).get("baseMetricV3", {})
        #severity = impact.get("baseSeverity", "Unknown")
        severity = data['CVE_Items'][0]['impact']['baseMetricV3']["cvssV3"]['baseSeverity']

        #description = cve_item.get("cve", {}).get("description", {}).get("description_data", {}).get("value", "No description")
        ...
        #score = cve_item.get("impact", {}).get("baseMetricV3", {}).get("baseScore", 0)
        ...
        #cvss_version = cve_item.get("impact", {}).get("baseMetricV3", {}).get("version", 0.0)
        cvss_version = impact.get("cvssV3", {}).get("version", 0.0)

        #cvss_vector = cve_item.get("impact", {}).get("baseMetricV3", {}).get("cvssV3", {}).get("vectorString", "Unknown")
        cvss_vector = impact.get("cvssV3", {}).get("vectorString", "Unknown")
