...
class CVEDataProcessor:
    def __init__(self, database_file="/home/escan/aaaa/cvebintool/cve.db"):
    ...
    async def process_all_cache_dirs(self):
        ...
        cache_dirs = [
            ...
            #"/home/escan/181023/data_source/curl",
            #"/home/escan/181023/data_source/gad",
            #"/home/escan/181023/data_source/osv",
            #"/home/escan/181023/data_source/redhat",
            #"/home/escan/181023/data_source/rsd",
        ]
    ...
    async def process_cache_dir(self, cache_dir):
        ...
        for file in Path(cache_dir).glob("*.[jJ][sS][oO][nN]*"):  # Match JSON and JSON.gz files
    ...
    def extract_severity_info(self, cve_item):
        ...
        description = cve_item.get("cve", {}).get("description", {}).get("description_data", [{}])[1].get("value", "No description available")
        score = cve_item.get("impact", {}).get("baseMetricV3", {}).get("cvssV3", {}).get("baseScore", 0)
        cvss_version = cve_item.get("impact", {}).get("baseMetricV3", {}).get("cvssV3", {}).get("version", 0.0)
        cvss_vector = cve_item.get("impact", {}).get("baseMetricV3", {}).get("cvssV3", {}).get("vectorString", "Unknown")
        data_source = "NVD"  # Default value
        last_modified = cve_item.get("lastModifiedDate", "Unknown")

        return cve_number, severity, description, score, cvss_version, cvss_vector, data_source, last_modified
    ...
    def insert_cve_data(self, json_data):
        ...
        for cve_item in json_data.get("CVE_Items", []):
            ...
            if not existing_entry:
                severity, description, score, cvss_version = this.extract_severity_info(cve_item)
